...
impl FileEngine {
    ...
    /// Create a FileEngine from a VMDK disk image file.
    ...
    pub fn update_file_path(&mut self, file: File) -> Result<(), BlockIoError> {
        match self {
            ...
            FileEngine::Vmdk(_) => {
                // For VMDK, we need to re-create the engine since imago parses the full
                // descriptor on open. Replace the current engine entirely.
